document.addEventListener('DOMContentLoaded', function () {
    ...
    async function fetchAndDisplayHistory() {
        ...
        // Caminho absoluto com a capitalização correta
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Falha na API: Status ${response.status}. Resposta do servidor: ${errorText}`);
            }
            ...
            if (messages.length === 0) {
                messageList.innerHTML = '<div class="text-center text-muted p-3">Seja o primeiro a enviar uma mensagem!</div>';
            } else {
    ...
    async function sendMessage() {
        ...
        try {
            ...
            if (!response.ok) {
                const errorText = await response.text();
                throw new Error(`Falha na API ao enviar: Status ${response.status}. Resposta: ${errorText}`);
            }
